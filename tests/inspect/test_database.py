...
def test_inspect_database_schema() -> None:
    connection = Connection("postgres", _get_connection_string())
...
def test_preview_database() -> None:
    ...
    results = preview_database(connection)
...
def test_preview_database_selected_columns() -> None:
    ...
    results = preview_database(
        connection, 5000, options=InspectOptions(["ts", "value"])
    )
    assert results is not None
    assert len(results) == 3
...
def test_preview_database_limit_rows() -> None:
    ...
    results = preview_database(connection, 2)
    assert results is not None
    assert len(results) == 2
...
def test_read_database() -> None:
    ...
    batches = list(read_database(connection))
