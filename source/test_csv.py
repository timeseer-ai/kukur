...
class TestCsv():
    ...
    def test_dir(self):
        source = from_config({'path': 'tests_kukur/test_data/csv/dir', 'format': 'dir'})
    ...
    def test_row(self):
        source = from_config({'path': 'tests_kukur/test_data/csv/row.csv', 'format': 'row'})
        table = source.get_data(self.series_id, self.start_date, self.end_date)
        assert len(table) == 5
        assert table.column_names == ['ts', 'value']
        assert table['ts'][0].as_py() == self.start_date
    ...
    def test_pivot(self):
        source = from_config({'path': 'tests_kukur/test_data/csv/pivot.csv', 'format': 'pivot'})
        table = source.get_data(self.series_id, self.start_date, self.end_date)
        assert len(table) == 7
        assert table.column_names == ['ts', 'value']
        assert table['ts'][0].as_py() == self.start_date
    ...
    def test_row_metadata_dictionary(self):
        source = from_config({
            'metadata': 'tests_kukur/test_data/csv/row-metadata.csv',
            'dictionary_dir': 'tests_kukur/test_data/csv/dictionary'
        })
