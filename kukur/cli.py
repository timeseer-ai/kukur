...
from kukur.flight import JSONFlightServer, KukurFlightServer, KukurServerAuthHandler, KukurServerNoAuthHandler


def parse_args():
    """Parse the command line arguments given to Kukur."""
    parser = argparse.ArgumentParser(description='Start Kukur.')
    parser.add_argument('--config-file', default='Kukur.toml', help='Path to the configuration file')
    subparsers = parser.add_subparsers(dest='action', help='Select the CLI action')

    subparsers.add_parser('flight', help='Enable the Arrow Flight interface (the default)')
    test_parser = subparsers.add_parser('test', help='Test data source connectivity')
    api_key_parser = subparsers.add_parser('api-key', help='Create an api key for the arrow flight interface')

    test_subparsers = test_parser.add_subparsers(dest='test_action', help='Select the type of data to test')
    search_parser = test_subparsers.add_parser('search', help='List all time series in the source')
    metadata_parser = test_subparsers.add_parser('metadata', help='Display metadata for one time series')
    data_parser = test_subparsers.add_parser('data', help='Display data for one time series')

    search_parser.add_argument('--source', required=True, metavar='SOURCE NAME',
                               help='The name of the data source to test')
    metadata_parser.add_argument('--source', required=True, metavar='SOURCE NAME',
                                 help='The name of the data source to test')
    metadata_parser.add_argument('--name', required=True, metavar='SERIES NAME',
                                 help='The name of the series to query')
    data_parser.add_argument('--source', required=True, metavar='SOURCE NAME',
                             help='The name of the data source to test')
    data_parser.add_argument('--name', required=True, metavar='SERIES NAME',
                             help='The name of the series to query')
    data_parser.add_argument('--start', required=True, metavar='START DATE',
                             help='The start date of the time period to query')
    data_parser.add_argument('--end', required=True, metavar='END DATE',
                             help='The end date of the time period to query')

    api_key_subparser = api_key_parser.add_subparsers(dest='api_key_action', help='Select the type of data to test')
    ...
    create_parser.add_argument('--name', required=True, metavar='API KEY NAME',
                               help='The name of the api key')
    revoke_parser.add_argument('--name', required=True, metavar='API KEY NAME',
                               help='The name of the api key')
...
def _serve(kukur_app: Kukur, server_config):
    ...
    if 'flight' in server_config and not server_config['flight'].get('authentication', True):
...
def _test_source(kukur_app: Kukur, args):
    ...
    if args.test_action == 'search':
        ...
        for row in test_source.data(kukur_app, source_name, series_name, start_date, end_date):
            writer.writerow(row)
