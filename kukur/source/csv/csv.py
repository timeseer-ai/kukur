...
def _read_row_data(loader: Loader, selector: SeriesSelector) -> pa.Table:
    ...
    convert_options = pyarrow.csv.ConvertOptions(column_types={'ts': pa.timestamp('us', 'utc')})
    all_data = pyarrow.csv.read_csv(loader.open(), read_options=read_options, convert_options=convert_options)
    # pylint: disable=no-member
