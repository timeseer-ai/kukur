...
class ParquetSource():
    ...
    def get_metadata(self, selector: SeriesSelector) -> Metadata:  # pylint: disable=no-self-use
...
def _read_pivot_data(loader: Loader, selector: SeriesSelector) -> pa.Table:
    ...
    return all_data.select([0, selector.name]).rename_columns(['ts', 'value']).cast(schema)


def _read_row_data(loader: Loader, selector: SeriesSelector) -> pa.Table:
    schema = pa.schema([('series name', pa.string()), ('ts', pa.timestamp('us', 'utc')), ('value', pa.float64())])
    all_data = parquet.read_table(loader.open()).rename_columns(['series name', 'ts', 'value']).cast(schema)
    # pylint: disable=no-member
    data = all_data.filter(pyarrow.compute.equal(all_data['series name'], pa.scalar(selector.name)))
