...
class KukurFlightServer():
    ...
    def get_data(self, _, request) -> Any:
        ...
        selector = SeriesSelector(request['selector']['source'], request['selector']['name'])
...
class KukurServerAuthHandler(fl.ServerAuthHandler):
    ...
    def authenticate(self, outgoing, incoming):
        ...
        if not self._app.get_api_keys() \
                .is_valid(auth.username.decode('UTF-8'), auth.password.decode('UTF-8')):
