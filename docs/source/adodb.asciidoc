...
The Timeseer installer includes pywin32 by default.
...
When provided, Timeseer does not need a series to have been used in another context before it can be analyzed.
...
The columns in the result set need to be mapped to the types of metadata known in Timeseer.
...
Timeseer will try to convert columns to the expected type.
