...
list_query = "" # optional: SQL query to list metadata
data_query = "" # optional: SQL query to fetch data for a time series
data_query_named_paramaters = {} # optional: mapping of named query parameters to tags
...
Data can be queried as well.
Two query parameters `startDate` and `endDate` are always provided.
Tags in the series selector should be mapped to query parameters using `data_query_named_parameters`.
...
type = "csv"
name = "series_data"
location = "tests/test_data/datafusion/series_data.csv"
"""

[[source.datafusion.data_query_named_parameters]]
name = "series name"
```
