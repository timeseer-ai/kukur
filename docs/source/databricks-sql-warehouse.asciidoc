...
Data for time series is queried using the `data_query`.
...
password = "" # optional: the Personal Access Token (PAT)
oauth_client_id = "" # optional: the client (application) id for OAuth2 m2m authentication
oauth_client_secret = "" # optional: the client secret for OAuth2 m2m authentication
azure_workspace_resource_id = "" # optional: the Azure resource (object) id of the Databricks workspace to connect to
...
The `data_query` returns one column containing the timestamps,
followed by one column for each field in `field_columns`,
in the same order.
When a `quality_mapping` is configured,
a quality column is expected as the last column.
The column of the requested field is returned as the `value` column.
...
Follow the steps in https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/oauth-m2m[Authorize service principal access to Azure Databricks with OAuth] to generate `oauth_client_id` and `oauth_client_secret`.

Multiple fields are supported by listing them in `field_columns`
and returning one column per field in the `data_query`:
...
The full documentation to achieve this is available at https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/azure-mi[Set up and use Azure managed identities authentication for Azure Databricks automation]
...
First follow the steps in the https://learn.microsoft.com/en-us/azure/databricks/dev-tools/auth/oauth-m2m[OAuth machine-to-machine authentication documentation].
...
==== Connecting to Databricks SQL Warehouses on Azure using Managed Identities (ODBC)
...
==== Connecting to Databricks SQL Warehouses using a Personal Access Token (ODBC)
...
==== Using Alternative Authentication Options (ODBC)
