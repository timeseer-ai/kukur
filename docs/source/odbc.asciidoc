...
autocommit = false
encoding = "" # optional encoding to use for queries and responses
...
While ODBC generally expects data to be encoded as UTF16LE,
databases or drivers can deviate from this.
Use the `encoding` option to configure a different encoding to use for queries and for reading textual data in responses.
...
to configure which values in the quality column are considered good.
