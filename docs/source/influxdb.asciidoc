...
verify_ssl = <boolean to indicate if the SSL certificate should be verified>
timeout_seconds = <number of seconds before a request times out>
...
Set `verify_ssl` to `false` when InfluxDB uses a self-signed certificate.
