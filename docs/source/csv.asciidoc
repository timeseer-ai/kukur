...
and where the files contain a header row. This configuration allows mapping
the columns from CSV files to the columns expected by Kukur.
...
the directory data model uses the CSV file names as the
series names.
