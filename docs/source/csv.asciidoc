...
Metadata lookups respect the `tag_columns` and `field_columns` options.
Only the first name in `field_columns` will be used for matching a field.
When `field_columns` is omitted,
matching is done on `tag`s only.
