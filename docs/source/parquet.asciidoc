...
In cases where the `row` or `pivot` format is used for pre-sorted data,
using a `type = "datafusion"` source will improve memory usage and performance,
because `datafusion` supports selectively reading the `parquet` file.
