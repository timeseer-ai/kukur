...
data models. This configuration allows mapping the columns of tables read from
parquet files to the columns expected by Kukur. This is done by setting the
...
`series name` is only valid for the `row` format, as
the directory data model uses the parquet file names as the
series names.
