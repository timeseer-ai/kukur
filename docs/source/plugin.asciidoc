...
When no quality mapping is configured for the source,
`1` is considered good and everything else is considered bad.

To provide other quality values,
...
.
The quality values of the plugin are returned as-is,
the mapping is embedded in the metadata of the Arrow schema.

Note that a quality column that has been simplified by `kukur.simplify_quality` uses the opposite convention,
where `0` means good.
