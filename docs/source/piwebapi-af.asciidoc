...
use_attribute_path = true # set to false to use the attribute name instead for the tag/field.
metadata_attributes = {} # optional: metadata field name to element attribute name
web_id_type = "Full" # one of Full, IDOnly, PathOnly
...
enable_trace_logging = false # optional: log all PI Web API requests and responses
...
`metadata_attributes` sets metadata fields of all series of an element to the value of an attribute of that element.
The keys are metadata field names,
the values are attribute names relative to the element.

For example,
this sets the `location` metadata field to the value of the `Location` attribute
and the `active` metadata field to the value of the `Active` child attribute of `Status`:

```toml
[source.<name>.metadata_attributes]
location = "Location"
active = "Status|Active"
```

The values are requested in a separate batch request after the elements have been found,
which uses `metadata_request_timeout_seconds`.
When a value lookup fails, a warning is logged and the metadata field is left empty.
Values that are not good are ignored.
...
`max_returned_metadata_items_per_call` controls how many elements matching a template - or attributes matching a category - are requested in one metadata batch call.
The default setting is `150`.
Decrease it when the batch endpoint runs into rate limit errors because the server has a low `max_returned_items_per_call`.
Kukur paginates through all matching items regardless of this setting.
...
Set `enable_trace_logging` to `true` to log all HTTP requests to PI Web API,
including the JSON sent and the JSON received.
This is useful to debug batch queries.
...
The `Good` flag of a data point is returned in the `quality` column as `0` (good) or `1` (bad).

PI system digital states (e.g. `Shutdown`, `No Data`, ...) are always included in the data.
Their `value` is `null`;
the numeric code of the digital state is returned as the `quality` value instead.
Check the
ifdef::sources[]
