...
include_system_states = false # optional: include digital states with IsSystem = true as bad quality data points
verify_ssl = true
...
The `Good` flag of a data point is returned in the `quality` column.
Since this is a flag and not a status code,
the column is already simplified:
`0` means good,
`1` means bad.
Check the
ifdef::sources[]
