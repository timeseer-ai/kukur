...
Use `kukur.simplify_quality` to reduce a quality column to good and bad.
It uses the embedded quality mapping,
so it works for every source.

```python
from kukur import Client, simplify_quality

client = Client()
data = simplify_quality(client.get_data(selector, start_date, end_date))
```
...
which makes calling `simplify_quality` again return the same data.

Sources that provide quality flags rather than status codes,
such as the PI Web API sources,
return a simplified quality column directly.
Calling `simplify_quality` on their data returns the same data.
...
or call `simplify_quality` first.
