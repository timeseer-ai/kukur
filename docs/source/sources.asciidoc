...
Sources that provide quality flags without a configured quality mapping,
such as the PI Web API sources,
return `1` for good data points.
Their embedded mapping is `{"GOOD": [1]}`.
...
WARNING: This is the opposite of the convention used by sources that provide quality flags,
where `1` means good.
The two are told apart by the type of the column:
a simplified quality column is `int8`,
while the raw quality column of a source is `int16` or `string`.
The `kukur.quality` metadata always describes the column in the same table:
after simplifying it is `{"GOOD": [0]}`.
