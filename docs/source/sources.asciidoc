...
"good" = [192, 194]
"sensor failure" = 401
"connection failure" = 402
```

In this example the quality of 192 and 194 is shown as "good" and 401 and 402 have specific error states.
...
Use `kukur.quality.describe` to replace the quality values of the data by their display values.
...
```json
{"GOOD": [192, [194, 200]], "display": {"good": [192, 194]}}
```

The `display` entry is only present when the source configures one.
The quality values that a display value names are always embedded as a list,
also when the source configures only one of them.
